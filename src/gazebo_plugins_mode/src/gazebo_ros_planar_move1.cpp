...
public:
  ...
  gazebo::physics::EntityPtr entity_;
...
GazeboRosPlanarMove1::GazeboRosPlanarMove1()
: impl_(std::make_unique<GazeboRosPlanarMove1Private>())
{
}
...
void GazeboRosPlanarMove1::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  impl_->model_ = _model;
...
//////////////////////////////////
  ...
  impl_->cmd_vel_sub_ = impl_->ros_node_->create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&GazeboRosPlanarMove1Private::OnCmdVel, impl_.get(), std::placeholders::_1));
  ...
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&GazeboRosPlanarMove1Private::OnUpdate, impl_.get(), std::placeholders::_1));
}

void GazeboRosPlanarMove1::Reset()
{
  impl_->last_update_time_ = impl_->world_->SimTime();
...
void GazeboRosPlanarMove1Private::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  double seconds_since_last_update = (_info.simTime - last_update_time_).Double();
// std::cout << "huhu" << std::endl;
  std::lock_guard<std::mutex> scoped_lock(lock_);
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("GazeboRosPlanarMove1Private::OnUpdate");
  ...
  if (seconds_since_last_update >= update_period_) {
    ...
    model_->SetLinearVel(
      ignition::math::Vector3d(
        ...
        target_cmd_vel_.linear.z * cosf(yaw) + target_cmd_vel_.linear.z * sinf(yaw)));
...
void GazeboRosPlanarMove1Private::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
{
  std::lock_guard<std::mutex> scoped_lock(lock_);
...
void GazeboRosPlanarMove1Private::UpdateOdometry(const gazebo::common::Time & _current_time)
{
  ...
  odom_.twist.twist.linear.z = cosf(yaw) * linear.Z() - sinf(yaw) * linear.Z();
  // Set timestamp
  odom_.header.stamp = gazebo_ros::Convert<builtin_interfaces::msg::Time>(_current_time);
}

void GazeboRosPlanarMove1Private::PublishOdometryTf(const gazebo::common::Time & _current_time)
{
...
GZ_REGISTER_MODEL_PLUGIN(GazeboRosPlanarMove1)
}  // namespace gazebo_plugins
